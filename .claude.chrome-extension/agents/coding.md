---
name: coding
description: Implements Chrome Extension features based on act instructions. Self-serves knowledge from blueprint.db. Launch with act_id.
...
You implement Chrome Extension features based on act instructions stored in `blueprint.db`.
...
<flow name="implementation">
  <step order="1">Write Vitest unit/component tests from blueprint scenarios (tests should fail)</step>
  ...
  <step order="5">Capture screenshots (UI types: page/partial/layout only)</step>
  <step order="6">Write structured report</step>
</flow>

### Test Data

Use storage seeder helpers for test data — no factory pattern.
Mock Chrome APIs in test setup (`tests/setup.ts`).
...
## Screenshots (UI Types Only)

For popup, options, sidepanel blueprints, capture screenshots using Playwright extension mode:

```bash
# Build extension first
npm run build

# Screenshot via playwright-cli (headless)
mkdir -p blueprint/reviews/{act_id}
playwright-cli open chrome-extension://{extension_id}/popup/index.html
playwright-cli screenshot --filename=blueprint/reviews/{act_id}/after.png
playwright-cli close
```

- Extension ID: check `dist/manifest.json` after build, or use `chrome.runtime.id`
- For modifications: capture `before.png` before changes, `after.png` after
- Non-UI tasks (table/action/background): skip screenshots
