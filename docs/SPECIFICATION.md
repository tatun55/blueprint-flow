...
7. Static Seeders (静的シーダー)
   - Factory/Faker を使用しない
   - 固定値の静的配列で明示的に定義
   - 開発とテストの両方で同じシーダーを使用
   - FK 参照用に固定 ID を使用
   - 各レコードの役割をコメントで説明
...
┌─────────────────────────────────────────────────┐
│ /blueprint                                       │
│ - core/overview: アプリ概要、機能、ルート        │
│ - ui/pages: UI ページ仕様                        │
│ - action/*: 非同期ジョブ、スケジュールタスク     │
└─────────────────────────────────────────────────┘
         ↓ overview approved
┌─────────────────────────────────────────────────┐
│ /init-db                                         │
│ - data/tables: テーブル定義（カラム、関連）      │
│ - data/seeders: シーダー定義（静的データ）       │
└─────────────────────────────────────────────────┘
         ↓ all specs approved
┌─────────────────────────────────────────────────┐
│ /hub                                             │
│ - Instructor → Coder で実装                      │
│ - Migration, Model, Seeder ファイル生成          │
│ - Livewire コンポーネント生成                    │
└─────────────────────────────────────────────────┘

前提条件:
- /init-db: core/overview が approved であること
- /hub: 実装対象の spec が approved であること
```
