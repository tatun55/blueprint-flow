...
echo "" | $HUB record-review {bp_id} approve {act_id} night-runner
$HUB complete {bp_id}
```
...
# Record rejection with failure summary
echo "test failures: {from report}, blueprint-match gaps: {from report}" | $HUB record-review {bp_id} reject {act_id} night-runner
